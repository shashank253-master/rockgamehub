...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'change_password') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        $pwd_user = $pwd_result->fetch_assoc();
        $stmt->close();
...
<div class="max-w-full px-4 py-6 space-y-6">
    ...
    <div>
        <h2 class="text-3xl font-bold mb-2">
            <i class="fas fa-user-circle text-purple-500 mr-2"></i>My Profile
        </h2>
    ...
    <div class="flex gap-2 border-b border-slate-700">
        <a href="?tab=profile" class="px-6 py-3 font-semibold transition-all <?php echo ($current_tab === 'profile') ? 'text-white border-b-2 border-purple-500' : 'text-gray-400 hover:text-white'; ?>">
    ...
    <?php if ($current_tab === 'profile'): ?>
        ...
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-8 shadow-lg">
            <h3 class="text-2xl font-bold mb-6">Account Information</h3>
            ...
            <form method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="username" value="<?php echo htmlspecialchars($user['username']); ?>" required class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" required class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition">
                </div>

                <div class="bg-slate-700/50 rounded-lg p-4 mt-6">
                    <p class="text-gray-400 text-sm mb-2">
                        <i class="fas fa-info-circle mr-2"></i>Wallet Balance
                    </p>
                    <p class="text-3xl font-bold text-green-400">₹<?php echo number_format($user['wallet_balance'], 2); ?></p>
                </div>
    ...
    <?php else: ?>
        ...
        <!-- Logout Section -->
        <div class="bg-red-900/20 border border-red-500 rounded-2xl p-8 mt-6">
            <h3 class="text-2xl font-bold mb-4 text-red-400">
                <i class="fas fa-sign-out-alt mr-2"></i>Session
            </h3>
            <p class="text-gray-300 mb-4">End your current session and log out of your account.</p>
